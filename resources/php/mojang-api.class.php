...
class MojangAPI
{
    ...
    public static function displayServerStatus() {
        $server = static::query("mcvanilla.me");
        $hostname = "mcvanilla.me";
        $motd = $server["motd"];
        $connection = 5;
        ...
        if (empty($server)) {
            $hostname = "mcvanilla.me";
            $motd = "<em>Can't connect to server</em>";
            $connection = 0;
            $numplayers = 0;
            $maxplayers = 0;
        }
        ...
        echo '
            ...
            <div class="connection-quality"><img src="resources/images/bars-' . $connection . '.png"</img></div>
            <div class="slots">' . $numplayers . '<em>/</em>' . $maxplayers . '</div>
            <br />
     ...
     */
    public static function getSkin($uuid, $dir)
    {
        $path = $dir . static::getUsername($uuid) . ".png";
        $skinUrl = static::getSkinUrl($uuid);
        if (file_exists($path))
            return file_get_contents($path);
        if (is_string($skinUrl)) {
            $data = static::fetch($skinUrl);
     ...
     */
    public static function getPlayerHead($uuid, $dir = "resources/images/playerskins/", $size = 100)
    {
        $skin = static::getSkin($uuid, $dir);
        if (is_string($skin)) {
            return static::getPlayerHeadFromSkin($skin, $size);
        } else
            return static::getSteveHead();
        return $skin;
    }
    ...
    public static function getAverage($sourceURL)
    {
